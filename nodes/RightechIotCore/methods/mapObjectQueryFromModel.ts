...
import { isConfigDescriptor, type RicModelDescriptor } from '../common/types.js';
import {
	httpCall,
	ricConfigToResourceMapperField,
	unrollModelDescriptors,
} from '../common/util.js';

export async function mapObjectQueryFromModel(
	...
	try {
		...
		return {
			fields: [...fields, ...configs.map(ricConfigToResourceMapperField)],
			emptyFieldsNotice:
				configs.length === 0 ? 'Model does not have any configuration fields.' : undefined,
		};
	} catch (error) {
		return {
			fields: [],
