...
it('No parameter reuse', () => {
    ...
    for (const node of nodes) {
        ...
        if (node.name === 'operation') {
            expect(show, `Operation parameter ${node.name} is declared with shared visibility, which is prone to duplication errors`).toMatchObject({
                resource: [expect.any(String)],
            });
        } else if (node.name !== 'resource') {
            expect(show, `Parameter ${node.name} is declared with shared visibility, which is prone to duplication errors`).toMatchObject({
                resource: [expect.any(String)],
...
it('All parameters correspond to defined operations', () => {
    ...
    for (const node of nodes) {
        if (node.name === 'resource') {
            ...
            for (const type of (node.options as INodePropertyOptions[]).map(o => o.value)) {
                expect(resources[byResource], `Operation ${type} is shown for resource "${byResource}" which is not declared`).toBeDefined();
                resources[byResource]?.push(type as string);
            }
        } else {
            const {resource: [byResource], operation: [byOperation]} = node.displayOptions?.show as {resource: [string], operation: [string]};
            expect(resources[byResource]?.includes(byOperation), `Property ${node.name} is shown for operation "${byResource}/${byOperation}" which is not declared`).toBe(true);
        }
    }
})
