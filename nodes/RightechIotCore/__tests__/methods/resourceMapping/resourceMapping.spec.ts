...
it('mapTelemetryParams - not selected', async () => {
	const options = await mapTelemetryParams.call(
		loadOptions(await import('./mapTelemetryParams.empty.workflow.json')),
	);
...
it('mapTelemetryParams', async () => {
	...
	const options = await mapTelemetryParams.call(
		loadOptions(await import('./mapTelemetryParams.workflow.json')),
	);
...
it('mapTableRowQuery', async () => {
	...
	const options = await mapTableRowQuery.call(
		loadOptions(await import('./mapTableRow.workflow.json')),
	);
	expectScopeDone(scope);
	expect(options).toMatchObject({
		fields: expect.any(Array),
...
it('mapObjectQueryFromModel', async () => {
	...
	const options = await mapObjectQueryFromModel.call(
		loadOptions(await import('./mapObjectQueryFromModel.workflow.json')),
	);
	expectScopeDone(scope);
	expect(options).toMatchObject({
		fields: expect.any(Array),
...
it('mapObjectColumnsFromModel', async () => {
	...
	const options = await mapObjectColumnsFromModel.call(
		loadOptions(await import('./mapObjectColumnsFromModel.workflow.json')),
	);
	expectScopeDone(scope);
	expect(options).toMatchObject({
		fields: expect.any(Array),
