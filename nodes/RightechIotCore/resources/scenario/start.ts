...
export const scenarioStartProperties: INodeProperties[] = [
	{
		...objectSelector,
		displayOptions,
	},
...
export async function start(exec: IExecuteFunctions, index: number): Promise<INodeExecutionData[]> {
	...
	const responseData = (await httpCall(exec, {
		method: 'POST',
		url: `/api/v1/objects/${objectId.value}/automatons/${scenarioId.value}/start`,
		headers: {
