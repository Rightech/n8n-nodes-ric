...
import {
    ...
    RicPerObjectModelDeepAndFlat, RicTelemetryDataTypes
        ...
        for (const prop of modelFlatData._arguments) {
            ...
            if (prop.id in input) {
                ...
                if (validateInputType(prop.dataType, value)) {
                    ...
                    throw new NodeOperationError(exec.getNode(), `Input data for parameter ${prop.id} did not match the schema - expected ${prop.dataType}, got ${Array.isArray(value) ? 'array' : typeof value}`);
                }
            }
        }
    }
