import {IDataObject, IExecuteFunctions, IHttpRequestOptions, INodeExecutionData,
...
export const objectGetHistoryProperties: INodeProperties[] = [
    ...
    {
        displayName: 'IoT devices can amass a lot of telemetry, so to keep your workflows efficient try to limit selected ranges to the most narrow you can use.',
...
export async function getHistory(exec: IExecuteFunctions, index: number): Promise<INodeExecutionData[]> {
    ...
    const historyQueryParameters = exec.getNodeParameter('historyQueryParameters', index) as historyQueryParameters;
    if (!!getHistoryTo && getHistoryTo < getHistoryFrom) {
        throw new WorkflowConfigurationError(exec.getNode(), `Lookup time range ${getHistoryFrom}-${getHistoryTo} is inverted - no data can be found.`);
    }
