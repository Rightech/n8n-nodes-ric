...
export const objectCreateProperties: INodeProperties[] = [
	...
	{
		displayName:
			'You can leave template parameters like <pre>`$tagname`</pre> or <pre>`$nanoid`</pre> as is to fill them with defaults dynamically.',
...
export async function create(
	...
	const defaultName =
		modelOptions.schema.find((f) => f.id === 'name')?.defaultValue?.toString() ?? '';
	const tagname =
		modelOptions.schema.find((f) => f.id === '@tagname')?.defaultValue?.toString() ?? '';
	const idPrefix =
